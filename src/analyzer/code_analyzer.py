...
class CodeAnalyzer:
    ...
    def analyze_relationships(self, file_paths: List[Path]) -> Dict[str, Any]:
        relationships = {
            "function_calls": [],
            "class_inheritance": [],
            "import_dependencies": []
        ...
        for file_path in file_paths:
            try:
                self.current_file = file_path
                # Use parse_file instead of trying to read the file again
                tree = self.parser.parse_file(str(file_path))
                if not tree:
                    print(
                        f"Skipping relationship analysis for {file_path}: Failed to parse")
                    continue
            ...
            if cursor.node.type == "call":
                ...
                if function_name:
                    function_calls.append({
                        "file": str(self.current_file),
                        "caller": function_name.text.decode('utf8'),
                        "line": cursor.node.start_point[0]
                    })
                    ...
                    for base in bases.children:
                        if base.type == "identifier":
                            class_inheritance.append({
                                "file": str(self.current_file),
                                "class": name_node.text.decode('utf8'),
